...
def coco2017Dataset(client_id=0, total_clients=1, datadir='~/', partition_dataset=True, train_bsz=32, test_bsz=32,
                   is_test=True):
    """
    ...
    training_set = CocoDataset(root=coco_train_dir, annFile=coco_ann_train_file, transform=transform)
    ...
    if is_test:
        val_set = CocoDataset(root=coco_val_dir, annFile=coco_ann_val_file, transform=transform)
