...
def coco2017Data    (client_id=0, total_clients=1, datadir='~/', partition_dataset=True, train_bsz=32, test_bsz=32,
    ...
    coco_ann_train_file = os.path.join(datadir, 'annotations', 'instances_train2017.json')
    ...
    training_set = CocoDatasetObject(root=coco_train_dir, annFile=coco_ann_train_file, transform=transform)
    ...
    if get_training_dataset:
        ...
        if is_test:
            val_set = CocoDatasetObject(root=coco_val_dir, annFile=coco_ann_val_file, transform=transform)
            val_loader = torch.utils.data.DataLoader(val_set, batch_size=test_bsz, shuffle=True, generator=g,
                                                     num_workers=4)
            del val_set
        else:
            val_loader = None
