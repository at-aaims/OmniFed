...
def aggregate_updates(ch, method, properties, body, total_clients):
    ...
    if aggregate_update is None:
        aggregate_update = {name: np.zeros_like(param) for name, param in model_update.items()}
    ...
    if recvd_clients == total_clients:
        aggregate_update = {name: param / recvd_clients for name, param in aggregate_update.items()}
...
class MessageQueueCommunicator(Communicator):
    ...
    def aggregate(self, msg, communicate_params=True):
        ...
        if self.id == 0:
            aggregate_callback = partial(aggregate_updates, total_clients=self.total_clients)
            self.channel.basic_consume(queue=self.queue_name, on_message_callback=aggregate_callback, auto_ack=True)
            try:
            ...
            if isinstance(msg, torch.nn.Module):
                if communicate_params:
                    model_update = {name: param.data.numpy() for name, param in msg.named_parameters()}
                else:
                    model_update = {name: param.grad.numpy() for name, param in msg.named_parameters()}
            else:
                model_update = msg
