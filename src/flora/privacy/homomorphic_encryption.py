...
class HomomorphicEncryption:
    ...
    def encrypt(self, model: torch.nn.Module, encrypt_grads=True):
        ...
        for name, param in model.named_parameters():
            if encrypt_grads:
                encrypted_updates[name] = ts.ckks_vector(self.context, param.grad.view(-1).tolist())
            else:
                encrypted_updates[name] = ts.ckks_vector(self.context, param.data.view(-1).tolist())

        return encrypted_updates

    def decrypt(
        self, model: torch.nn.Module, encrypted_updates: Dict, encrypt_grads=True
    ):
