...
def run_server():
    ...
    try:
        process = subprocess.Popen([sys.executable, "scalable_parameter_server.py"])
        return process
...
def run_client(client_id, delay=0):
    ...
    try:
        process = subprocess.Popen([sys.executable, "scalable_client.py", client_id])
