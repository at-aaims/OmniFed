...
class ParameterServerServicer(parameter_server_pb2_grpc.ParameterServerServicer):
    def __init__(
        self, num_clients: int, model: nn.Module, keep_history_rounds: int = 1
    ):
        ...
        self.averaged_models_history = {}  # Keep history of averaged models
        self.keep_history_rounds = keep_history_rounds  # How many rounds to keep
    ...
    def GetAveragedModel(self, request, context):
        ...
        with self.lock:
            ...
            if round_number in self.averaged_models_history:
                # Get the averaged model for this round
    ...
    def _compute_averaged_model(self, round_number):
        ...
        self.averaged_model = averaged_params  # Keep current model reference
        print(f"Averaged model computed for round {round_number}")

    def _cleanup_old_rounds(self, current_round):
        """Clean up old rounds to prevent memory leaks"""
        rounds_to_remove = []

        # Remove client updates from old rounds
        for round_num in self.client_updates.keys():
            if round_num < current_round - self.keep_history_rounds:
                rounds_to_remove.append(round_num)

        for round_num in rounds_to_remove:
            del self.client_updates[round_num]
            print(f"Cleaned up client updates for round {round_num}")

        # Remove old averaged models
        rounds_to_remove = []
        ...
        for round_num in rounds_to_remove:
            ...
            print(f"Cleaned up averaged model for round {round_num}")
    ...
    def _protobuf_to_model_params(self, proto_layers):
        ...
        for layer in proto_layers:
            ...
            # Reshape weights and biases - convert RepeatedScalarContainer to tuple
...
def serve(port=50051, num_clients=3):
    ...
    model = SimpleModel()
    keep_history_rounds = 0

    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    parameter_server_pb2_grpc.add_ParameterServerServicer_to_server(
        ParameterServerServicer(num_clients, model, keep_history_rounds), server
    )
