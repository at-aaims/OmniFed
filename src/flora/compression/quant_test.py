...
class GradientQuantizer:
    ...
    def quantize_gradients(self, gradients: List[torch.Tensor]) -> Dict:
        ...
        quantized_data = {"tensors": [], "scales": [], "zero_points": [], "shapes": []}
...
def all_reduce_quantized_gradients(
    gradients: List[torch.Tensor], quantizer: GradientQuantizer
):
    """
...
def all_reduce_quantized_gradients_advanced(
    gradients: List[torch.Tensor], quantizer: GradientQuantizer
):
    """
...
def main():
    ...
    parser.add_argument(
        "--bit_width", type=int, default=8, help="Quantization bit width (default: 8)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--world_size", type=int, default=2, help="Number of GPUs (default: 2)"
    )
    ...
    if torch.cuda.device_count() < args.world_size:
        print(
            f"Not enough GPUs. Required: {args.world_size}, Available: {torch.cuda.device_count()}"
        )
        return
