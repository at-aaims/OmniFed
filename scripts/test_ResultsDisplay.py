...
from src.flora.utils.ExperimentDisplay import ExperimentResultsDisplay
...
class ExperimentDebugger:
    ...
    def get_experiment_dir_by_index(self, index: int) -> Path:
        ...
        if index < 0 or index >= len(experiment_dirs):
            raise IndexError(f"Invalid experiment index. Available: 0-{len(experiment_dirs)-1}")
    ...
    def extract_experiment_metadata(self, results: List[Dict[str, Any]], results_dir: Path) -> Tuple[float, int]:
                ...
                for context_name, context_data in node_result.items():
                    if context_data and isinstance(context_data, list):
                        ...
                        for measurement in context_data:
                            if isinstance(measurement, dict) and 'round_idx' in measurement:
                                rounds_seen.add(measurement['round_idx'])
                        
                        if rounds_seen:
                            global_rounds = max(rounds_seen) + 1  # Convert from 0-based to count
                            break
        
        # Try to find experiment config or metadata files
        experiment_root = results_dir.parent.parent if results_dir.parent.parent.exists() else results_dir.parent
    ...
    def run_debug_display(self, experiment_index: Optional[int] = None, 
                         validate_only: bool = False,
                         filter_metrics: Optional[List[str]] = None,
                         filter_context: Optional[str] = None) -> None:
        ...
        try:
            ...
            # Extract metadata
            duration, global_rounds = self.extract_experiment_metadata(results, results_dir)
            ...
            print("Features tested: coordinate columns, table styling, separators, data validation")
            print("=" * 80)
            
        except Exception as e:
            self._print_error_diagnostics(e)
    
    def _print_experiment_info(self, results: List[Dict[str, Any]], duration: float, global_rounds: int) -> None:
            ...
            if contexts:
                ...
                if results[0][first_context]:
                    ...
                    if isinstance(sample_measurement, dict):
                        metrics = [k for k in sample_measurement.keys() 
                                 if not k.startswith('_') and k not in ['round_idx', 'epoch_idx', 'batch_idx', 'global_step']]
                        print(f"└── Sample metrics in '{first_context}': {', '.join(metrics[:5])}")
                        if len(metrics) > 5:
    ...
    def _validate_basic_structure(self, results: List[Dict[str, Any]]) -> None:
        ...
        for i, node_result in enumerate(results[1:], 1):
            ...
            if node_contexts != first_contexts:
                ...
                if missing:
                    self.validation_warnings.append(f"Node {i} missing contexts: {missing}")
                if extra:
                    self.validation_warnings.append(f"Node {i} has extra contexts: {extra}")
    ...
    def _validate_data_consistency(self, results: List[Dict[str, Any]]) -> None:
        ...
        for node_result in results:
            for context_name, context_data in node_result.items():
                if context_data and isinstance(context_data, list):
                    context_counts[context_name] = context_counts.get(context_name, 0) + len(context_data)
        ...
        print(f"✓ Data consistency: {total_measurements} total measurements across {len(context_counts)} contexts")
                ...
                if context_data and isinstance(context_data, list):
                    ...
                    for measurement in context_data:
                        if isinstance(measurement, dict):
                            coord = (measurement.get('round_idx'), measurement.get('epoch_idx'), 
                                   measurement.get('batch_idx'), measurement.get('global_step'))
                    ...
                    if len(unique_coords) != len(coordinates_seen):
                        ...
                        coordinate_issues.append(f"{context_name}: {duplicates} duplicate coordinates")
    ...
    def _validate_metric_values(self, results: List[Dict[str, Any]]) -> None:
                    ...
                    for measurement in context_data:
                        if isinstance(measurement, dict):
                            for key, value in measurement.items():
                                if (key not in ['round_idx', 'epoch_idx', 'batch_idx', 'global_step'] 
                                    and not key.startswith('_') and isinstance(value, (int, float))):
                                    
                                    # Track metric statistics
                                    if key not in metric_stats:
                                        metric_stats[key] = {'values': [], 'context': context_name}
                                    metric_stats[key]['values'].append(value)
                                    
                                    # Check for suspicious values
                                    if value != value:  # NaN check
                                        suspicious_values.append(f"{context_name}.{key}: NaN value")
                                    elif abs(value) > 1e6:
                                        suspicious_values.append(f"{context_name}.{key}: Very large value ({value})")
                                    elif value < 0 and key.endswith(('accuracy', 'precision', 'recall')):
                                        suspicious_values.append(f"{context_name}.{key}: Negative accuracy metric ({value})")
        ...
        if suspicious_values[:5]:  # Show first 5 suspicious values
            ...
            if len(suspicious_values) > 5:
                self.validation_warnings.append(f"... and {len(suspicious_values)-5} more suspicious values")
        
        print(f"✓ Metric validation: {len(metric_stats)} metrics checked, {len(suspicious_values)} suspicious values")
    ...
    def _validate_percentage_calculations(self, results: List[Dict[str, Any]]) -> None:
        ...
        for node_result in results:
            for context_name, context_data in node_result.items():
                if context_data and isinstance(context_data, list) and len(context_data) >= 2:
                    progression_count += 1
        
        print(f"✓ Percentage calculation readiness: {progression_count} contexts have progression data")
    ...
    def _apply_filters(self, results: List[Dict[str, Any]], 
                      ...
                      filter_metrics: Optional[List[str]] = None) -> List[Dict[str, Any]]:
                ...
                if filter_metrics and context_data:
                    ...
                    for measurement in context_data:
                        if isinstance(measurement, dict):
                            ...
                            for key in ['round_idx', 'epoch_idx', 'batch_idx', 'global_step']:
                            ...
                            if len(filtered_measurement) > 4:  # More than just coordinates
    ...
    def run_comparison(self, exp1_index: int, exp2_index: int,
                      validate_only: bool = False,
                      filter_metrics: Optional[List[str]] = None,
                      filter_context: Optional[str] = None) -> None:
        ...
        try:
            ...
            exp1_duration, exp1_rounds = self.extract_experiment_metadata(exp1_results, exp1_dir)
            
            print("\n" + "-" * 40)
            
            exp2_dir = self.get_experiment_dir_by_index(exp2_index)
            exp2_results = self.load_node_results(exp2_dir)
            exp2_duration, exp2_rounds = self.extract_experiment_metadata(exp2_results, exp2_dir)
            
            # Compare basic info
            print("\n📋 Comparison Summary:")
            print(f"Experiment 1: {self._extract_experiment_name(exp1_dir)}")
            print(f"├── Nodes: {len(exp1_results)}, Rounds: {exp1_rounds}, Duration: {exp1_duration:.1f}s")
            print(f"Experiment 2: {self._extract_experiment_name(exp2_dir)}")
            print(f"└── Nodes: {len(exp2_results)}, Rounds: {exp2_rounds}, Duration: {exp2_duration:.1f}s")
            ...
            if not validate_only:
                # Apply filters
                if filter_context or filter_metrics:
                    exp1_results = self._apply_filters(exp1_results, filter_context, filter_metrics)
                    exp2_results = self._apply_filters(exp2_results, filter_context, filter_metrics)
...
Examples:
  ...
  python debug.py --outputs custom/                 # Use custom outputs directory
    ...
    parser.add_argument(
        '--context',
        help='Show only specific context (e.g., --context training)'
    )
    ...
    if args.compare:
        debugger.run_comparison(args.compare[0], args.compare[1], 
    ...
    # Run debug display
    debugger.run_debug_display(args.experiment, 
                              validate_only=args.validate_only,
                              filter_metrics=args.metrics,
                              filter_context=args.context)
